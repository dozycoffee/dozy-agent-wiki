...
export type PushResult =
  | { file: string; ok: true; slug: string; title: string; version: number }
  | { file: string; ok: false; status?: number; error: string };
...
async function pushOne(filePath: string, ctx: UploadContext): Promise<PushResult> {
  if (!/\.md$/i.test(filePath)) {
    return { file: filePath, ok: false, error: "only .md files are supported" };
  }
  ...
  if (!res.ok) {
    const body = (await res.json().catch(() => ({}))) as { error?: string };
    return { file: filePath, ok: false, status: res.status, error: body.error ?? `HTTP ${res.status}` };
  }

  const body = (await res.json()) as { slug: string; title: string; version: number };
  return { file: filePath, ok: true, slug: body.slug, title: body.title, version: body.version };
}
